...
/* fn decls */
struct node *createdbl(struct node *);
...
void main()
{
	struct node *dbl;
	int choice;
	dbl = NULL;  /* v. imp - LL is initialised */
	while (1)
	{
		clrscr();
		...
		printf("3. To Display the LL in Reverse Order\n");
		printf("4. Exit\n\n");
...
struct node *createdbl(struct node *dbl)
{
...
{
	/* to go to last node of LL */
	while (temp->next != NULL)
		temp = temp->next;

	/* now traverse in reverse direction & print */
	while (temp != NULL)
	{
		printf("%d\t",temp->data);
		temp = temp->prev;
